...
class Admin extends Core
{
    ...
    public function _initialize()
    {
        ...
        $name = $admin_menu->where('name',$this->title.'$menu=3')->find();
        if (empty($name) && $request->controller() != 'Index'){
            $name = $admin_menu->where('name',$this->title)->find();
        }
        ...
        $this->model_name = $request->controller();
    ...
    {
        $model = Loader::model($request->controller());
        //分页
    ...
    public function setStatus(Request $request,$model = '', $script = false)
    {
        ...
        switch ($status){
            case 'del':
                ...
                if ($info){
                    return $this->result($info,config('status.yes'),'成功删除'.$info.'条记录');
                }else{
                    return $this->result('',config('status.no'),'删除失败');
                }
                break;
            case config('status.open'):
                $info = Db::name($model)->where('id','IN',$id)->setField($field,config('status.close'));
                if ($info){
                    return $this->result($info,config('status.yes'),'操作成功'.$info.'条记录');
                }else{
                    return $this->result('',config('status.no'),'操作失败');
                }
                break;
            ...
            case 'auditrue':
                ...
                if ($info){
                    return $this->result($info,config('status.yes'),'审核通过'.$info.'条记录');
                }else{
                    return $this->result('',config('status.no'),'操作失败');
                }
                break;
            ...
            case 'auditfalse':
                ...
                if ($info){
                    return $this->result($info,config('status.yes'),'审核失败'.$info.'条记录');
                }else{
                    return $this->result('',config('status.no'),'操作失败');
                }
                break;
            default:
                return $this->result('',config('status.no'),'非法参数');
                break;
        }
    }
