...
class AliTarget{
    ...
    protected $receiveClass;

    public function __construct()
    {
        $this->accessKeyId = 'LTAIcPk2X5dbVBfZ';
        $this->accessKeySecret = 'hBGDp8WFsHaYB45FBMuM1G6XL5SwMB';
        $this->receiveClass = [];
    }

    /**
     * @param string $type 推送设备类别 andriod or ios
     * @param string $ios_value  ios设备号 多个用逗号分割(帐号与设备有一次最多100个的限制)
     * @param string $android_value  android设备号 多个用逗号分割(帐号与设备有一次最多100个的限制)
     * @param string $msgtype  推送类型 MESSAGE NOTICE
     * @param string $title 推送标题
     * @param string $content 推送内容
     * @param string $img ios推送图片url 如 http://waimai.023wx.cn/public/uploads/api/20180630/3d6dc19cd23b6945691d5825ab356640.jpg
     */
    public function alimsg($mobiltype=null, $ios_value=null,$android_value=null, $msgtype=null, $title=null, $content=null, $img=null)
    {
        include '../extend/php-aliyun-ams/aliyun-php-sdk-core/Config.php';
        $list=array();
        if($mobiltype == 'andriod') {
            $list['target'] = 'ACCOUNT';//推送目标: DEVICE:推送给设备; ACCOUNT:推送给指定帐号,TAG:推送给自定义标签; ALL: 推送给全部
            $list['target_value'] =$android_value;//测试： '13271828202'  根据Target来设定，如Target=device, 则对应的值为 设备id1,设备id2. 多个值使用逗号分隔.(帐号与设备有一次最多100个的限制)
            $list['parameterandriod']="{'attachment':'".$img."'}";
        }
        if($mobiltype == 'ios') {
            $list['target'] = 'DEVICE';//推送目标: DEVICE:推送给设备; ACCOUNT:推送给指定帐号,TAG:推送给自定义标签; ALL: 推送给全部
            $list['target_value'] = $ios_value; //  测试：'21ea0d4142b84ad9ac19dc27ee397a99'  根据Target来设定，如Target=device, 则对应的值为 设备id1,设备id2. 多个值使用逗号分隔.(帐号与设备有一次最多100个的限制)
            $list['parameterios']="{'attachment':'".$img."'}";
        }

        $list['push_type']=$msgtype;//消息类型 MESSAGE NOTICE
        $list['title']=$title;
        $list['content']=$content;
        $list['mobiltype']=$mobiltype;
        $this->pushMsg($list);
    }
...
//        $request->setAndroidExtParameters("{\"k1\":\"android\",\"model\":\"".$model."\",\"type\":\"".$type."\"}"); // 设定android类型设备通知的扩展属性
            $request->setAndroidExtParameters($list['parameterandriod']); // 设定android类型设备通知的扩展属性
